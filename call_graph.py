    ...
    def output(self, filename):
        ...
        if not filename.endswith(".dot"):
            if filename in ("", "."):
                ...
                filename += "."
            all_contracts_filename = f"{filename}all_contracts.call_graph.dot"
        ...
        with open(all_contracts_filename, "w", encoding="utf8") as f:
            ...
            function_duplicate_set=set()
            function_duplicate_set1=set()
            function_duplicate_set_add=function_duplicate_set.add
            function_duplicate_set1_add=function_duplicate_set1.add
            for function in all_functions:
                if function in function_duplicate_set_add:
                    function_duplicate_set1_add(function)
                else:
                    function_duplicate_set_add(function)
            function_list=list(function_duplicate_set1_add)
            all_functions_as_dict = {
                function.canonical_name+function_list.index(function)+"": function.full_name for function in function_list
            }
            content = "\n".join(
        ...
        for derived_contract in self.slither.contracts_derived:
            derived_output_filename = f"{filename}{derived_contract.name}.call_graph.dot"
